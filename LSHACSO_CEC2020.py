...
def RunLSHACSO(func):
    ...
    np.savetxt("./LSHACSO_Data/CEC2020/F" + str(FuncNum) + "_" + str(Dim) + "D.csv", All_Trial, delimiter=",")
...
def main(dim):
    ...
    for i in range(len(CEC2020)):
        FuncNum = i + 1
        RunLSHACSO(CEC2020[i].evaluate)


if __name__ == "__main__":
    if os.path.exists('./LSHACSO_Data/CEC2020') == False:
        os.makedirs('./LSHACSO_Data/CEC2020')
    Dims = [30, 50]
    for Dim in Dims:
        main(Dim)